...
async def test_initial_status_update_event(i3api, statusevents):
    assert statusevents == [(i3api, 'i3hub::status_update', [])]
...
async def test_i3bar_click_event(i3barmock, i3api, statusevents):
    ...
    assert statusevents[1] == (i3api, 'i3hub::status_click', [1,2,3])
    i3barmock.send_click(b',["click!"]\n')
    await asyncio.sleep(0.01)
    assert statusevents[2] == (i3api, 'i3hub::status_click', ['click!'])
    i3barmock.send_click(b',[""]\n')
    await asyncio.sleep(0.01)
    assert statusevents[3] == (i3api, 'i3hub::status_click', [''])
